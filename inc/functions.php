<?php
	 ...
	 */
	function verbalize_alert_level( $alert_level ){
		switch( $alert_level ){
	...
	function get_scan( $num_results = 1 ){
		...
		if( $query = $mysqli->prepare( "SELECT cpu_usage, sent_slack, sent_email FROM ". DB_TABLE ." ORDER BY scan_id DESC LIMIT ?" ) ){
	...
	function save_scan( $cpu_usage = 0, $one_min_avg = 0, $five_min_avg = 0, $fifteen_min_avg = 0, $sent_slack = false, $sent_email = false ){
		...
		if( $query = $mysqli->prepare( "INSERT INTO ". DB_TABLE ." (cpu_usage, one_min_avg, five_min_avg, fifteen_min_avg, scan_time, sent_slack, sent_email) VALUES (?, ?, ?, ?, ?, ?, ?)" ) ){
	...
	function delete_scans(){
		...
		if( $query = $mysqli->query( "SELECT COUNT(*) FROM ". DB_TABLE ."" ) ){
		...
		if( $total_rows > DB_ROW_LIMIT ){
			...
			if( $query = $mysqli->prepare( "DELETE FROM ". DB_TABLE ." ORDER BY scan_id ASC LIMIT ?" ) ){
	 ...
	 */
	function slack_handler( $channel = false, $message = false ){
		global $load_monitor;
